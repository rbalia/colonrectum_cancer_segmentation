...
def encoderBlock(input, num_filters, kernel_size):
    ...
    conv_block_decomp = convLayer(conv_block_decomp, num_filters, kernel_size)
...
def convBlock(input, num_filters, kernel_size):
    ...
    # CONV 2
    conv2_1 = convLayer(input, num_filters, 3)
    conv2_2 = convLayer(conv2_1, num_filters, 3)
    conv2_2 = Add()([conv2_2, conv1_1])

    # CONV 3
    conv3_1 = convLayer(conv2_2, num_filters, 5)
    conv3_2 = convLayer(conv3_1, num_filters, 5)
    conv3_2 = Add()([conv3_2, conv2_2])

    #merge = Concatenate()([conv1_1, conv2_2, conv3_2, conv4_2])

    #dense = Dense(num_filters, activation=None, kernel_initializer="he_uniform")(merge)
    #dense = BatchNormalization()(dense)
    #dense = Activation(activation="relu")(dense)
    #merge = convLayer(merge, num_filters, 1)
...
def build_model(inp_shape, out_channels=1):
    ...
    enc4, conv4 = encoderBlock(enc3, filters * 8, k_size)

    ## Bridge
    #brdg = bridgeBlock(enc4, filters * 8, k_size)

    # Upsample Phase
    dec1 = decoderBlock(enc4, conv4, filters * 8, k_size)
