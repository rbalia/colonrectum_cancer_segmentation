...
import plotter3D
from src.augmentation3D import image_augmentation
from src_analisys_modules.preprocessing import extract_subcubes, compute_subcubes_count, filter_empty_cubes, \
    split_valid_empty, count_ditribution
...
def datagenTo2DArray(datagen, slice_axis=0):
    ...
    for i, ds in enumerate(datagen):
        ...
        for slice_id in range(0, img.shape[slice_axis]):
            ...
            msk_slice = np.take(msk, slice_id, axis=slice_axis)#msk[:, :, slice_id, 0]
            #msk_slice[msk_slice > 0.5] = 1
            #msk_slice[msk_slice < 0.5] = 0
            #msk_slice = msk_slice / np.max(msk_slice)
            #msk_slice = np.expand_dims(msk_slice, axis=-1)
            Y.append(msk_slice)

            #plotter.dinamicFigurePlot("",["1","2"],[img_slice,msk_slice],shape=(1,2))
...
def datagenTo3DArray(datagen):
    print(f"DataGenerator length: {len(datagen)}")
    X = []
    Y = []
...
class DataGeneratorAOU_fromList(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            # Store sample
            print(f"ID:{ID} - {self.X_path_list[ID]}")
...
class DataGeneratorAOU_fixedSize_balanced_3(tf.keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, X_path_list, Y_path_list, batch_size=32, subs_cube_shape=(64,64,64), n_channels=1,
        ...
        self.subs_per_cube = compute_subcubes_count(full_cube_shape, subs_cube_shape, stride)
        self.subs_per_buffer = self.subs_per_cube * self.n_cubes
        self.X_buffer = np.empty((self.subs_per_buffer, *self.subs_cube_shape, self.n_channels), dtype=np.float32)
        self.Y_buffer = np.empty((self.subs_per_buffer, *self.subs_cube_shape, self.n_channels), dtype=np.float32)
        self.on_epoch_end()
    ...
    def __getitem__(self, batch_id):
        ...
        if ((batch_id_ + 1) * self.batch_size > len(self.X_buffer)) or batch_id_ == 0:
            ...
            # TODO: Fill with random samples if the batch of cubes is not large enough
    ...
    def __fill_buffer(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            # Load sample
            x = np.load(self.X_path_list[ID], mmap_mode='r').astype(np.float32)
            x = resize(x, (self.full_cube_shape), mode='constant')
            x = x / np.max(x) # Normalization

            # Load ground truth
            y = np.load(self.Y_path_list[ID], mmap_mode='r').astype(np.float32)
            y = resize(y, (self.full_cube_shape), order=0, anti_aliasing=False)
